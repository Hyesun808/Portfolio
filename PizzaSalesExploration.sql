...
from (select order_id, sum(quantity) as Number_of_pizzas
	  from PizzaSalesPortfolio.dbo.OrderDetails 
...
(select month(O.date) as Month, O.order_id, sum(quantity) as Number_of_pizzas
...
select pizza_type_id, name, trim(value) as Ingredient, 
	case when trim(value) like '%Sauce%' then 'Sauce' when trim(value) like '%Cheese%' then 'Cheese' 
		 when trim(value) in ('Anchovies','Bacon','Barbecued Chicken','Beef Chuck Roast','Calabrese Salami','Capocollo','Chicken','Chorizo Sausage',
							'Genoa Salami','Italian Sausage','Luganega Sausage','Nduja Salami','Peperoncini verdi','Pepperoni','Prosciutto',
							'Prosciutto di San Daniele','Sliced Ham','Soppressata Salami','Pancetta','Coarse Sicilian Salami') then 'Topping (Meat/Fish)'
		 when trim(value) in ('Artichoke','Arugula','Caramelized Onions','Cilantro','Corn','Eggplant','Friggitello Peppers','Garlic',
							'Green Peppers','Jalapeno Peppers','Mushrooms','Onions','Plum Tomatoes','Red Onions','Red Peppers','Spinach','Sun-dried Tomatoes',
							'Tomatoes','Zucchini') then 'Topping (Vegetables)'
		 else 'Topping (Others)' end as Ingredient_category
from PizzaSalesPortfolio.dbo.PizzaTypes
cross apply string_split(ingredients, ',')
order by Ingredient_category, Ingredient
...
select pizza_type_id, name, category, replace(trim(value), 'Artichokes','Artichoke') as Ingredient, 
	  case when trim(value) like '%Sauce%' then 'Sauce' when trim(value) like '%Cheese%' then 'Cheese' 
		 when trim(value) in ('Anchovies','Bacon','Barbecued Chicken','Beef Chuck Roast','Calabrese Salami','Capocollo','Chicken','Chorizo Sausage',
							'Genoa Salami','Italian Sausage','Luganega Sausage','Nduja Salami','Peperoncini verdi','Pepperoni','Prosciutto',
							'Prosciutto di San Daniele','Sliced Ham','Soppressata Salami','Pancetta','Coarse Sicilian Salami') then 'Topping (Meat/Fish)'
		 when trim(value) in ('Artichoke','Artichokes','Arugula','Caramelized Onions','Cilantro','Corn','Eggplant','Friggitello Peppers','Garlic',
							'Green Peppers','Jalapeno Peppers','Mushrooms','Onions','Plum Tomatoes','Red Onions','Red Peppers','Spinach','Sun-dried Tomatoes',
							'Tomatoes','Zucchini') then 'Topping (Vegetables)'
		 else 'Topping (Others)' end as Ingredient_category
...
select * from PizzaSalesPortfolio.dbo.PizzaTypesRevised order by Ingredient_category, Ingredient
